...
```mermaid
flowchart LR
    A["Lire la théorie"] --> B["Étudier la méthode"]
    B --> C["Refaire les exemples à la main"]
    C --> D["Exercices : chercher, puis l'indice"]
    D --> E["Comparer avec la solution détaillée"]
    E --> F{"Réussi ?"}
    F -- "Non" --> B
    F -- "Oui" --> G["Chapitre suivant"]
```
